@if($model->enabled)
    <div class="star-item" data-stars="{{$model->stars}}" data-type="{{$model->type}}" data-grade="{{$model->grade}}" data-rating="{{$model->rating}}" data-count="{{$model->voted}}" data-model="{{$model->model_class}}" data-model-id="{{$model->id}}">
        {!! $items !!}
    </div>
@endif
